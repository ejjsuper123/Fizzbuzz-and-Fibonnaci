...
public class FizzBuzz {
    ...
    public static String[] messages(int start, int end) throws IllegalArgumentException {
    	...
    	int fizzLength = (end - start);
    	ArrayList<String> fizzbuzzList = new ArrayList<>();
    	String list[] = new String[fizzLength];
    	
    	for (int i = 0; i < fizzLength; i++, start++) {
            if (message(start) != null)
                fizzbuzzList.add(message(start));
        }
    	list = fizzbuzzList.toArray(new String[fizzbuzzList.size()]);
